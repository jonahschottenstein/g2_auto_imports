...
/* const NavBar = () => {
	...
	const linkStyles = (href: string) => {
		return pathName === href
			? "nav-menu-link relative inline-block text-black focus:outline-none before:absolute before:bottom-0.5 before:start-0 before:-z-[1] before:w-full before:h-1 before:bg-lime-400"
			: "nav-menu-link inline-block text-black hover:text-gray-600 focus:outline-none focus:text-gray-600 dark:text-white dark:hover:text-neutral-300";
	};

	const handleCloseMenu = (e: React.MouseEvent<HTMLLinkElement>) => {
		const target = e.target as HTMLLinkElement;
		if (!e.target.matches("nav-menu-link")) return;
	};
	...
	return (
		<header className="flex flex-wrap md:justify-start md:flex-nowrap z-50 w-full py-7">
			<nav className="relative max-w-7xl w-full flex flex-wrap md:grid md:grid-cols-12 basis-full items-center px-4 md:px-6 lg:px-8 mx-auto">
				...
				<div
					id="hs-navbar-hcail"
					className="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow md:block md:w-auto md:basis-auto md:order-2 md:col-[none_/_span_6]"
					aria-labelledby="hs-navbar-hcail-collapse">
					<div className="font-sans flex flex-col gap-y-4 gap-x-0 mt-5 md:flex-row md:justify-center md:items-center md:gap-y-0 md:gap-x-7 md:mt-0">
						<div>
